...
<button class="btn btn-sm variant-ghost-surface" on:click={()=>{username = ""}}>Reset</button>

<h1 aria-label="bouncing">
    {#each letters as letter,i}
        <span style="--i: {i+1}" aria-hidden="true"><span  on:click={()=>{username+=letter}} >{letter}</span></span>
     {/each}
</h1>


</div>
<button class="btn btn-sm variant-ghost-surface">Login</button>
